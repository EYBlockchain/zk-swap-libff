...
{
    // return bw6_761_Fq(bw6_761_twist_mul_by_b_c0 * elt.c0, bw6_761_twist_mul_by_b_c1 * elt.c1, bw6_761_twist_mul_by_b_c2 * elt.c2);
    return bw6_761_G2::coeff_b * elt;
}
...
{
    return bw6_761_G2(this->X_, -(this->Y_), this->Z_);
}
...
bw6_761_G2 bw6_761_G2::mixed_add(const bw6_761_G2 &other) const
{
    ...
    const bw6_761_Fq u = Y2Z1 - this->Y_;             // u = Y2*Z1-Y1
    const bw6_761_Fq uu = u.squared();                // uu = u2
    const bw6_761_Fq v = X2Z1 - this->X_;             // v = X2*Z1-X1
    const bw6_761_Fq vv = v.squared();                // vv = v2
    const bw6_761_Fq vvv = v*vv;                      // vvv = v*vv
    const bw6_761_Fq R = vv * this->X_;               // R = vv*X1
    const bw6_761_Fq A = uu * this->Z_ - vvv - R - R; // A = uu*Z1-vvv-2*R
    const bw6_761_Fq X3 = v * A;                      // X3 = v*A
    const bw6_761_Fq Y3 = u*(R-A) - vvv * this->Y_;   // Y3 = u*(R-A)-vvv*Y1
    const bw6_761_Fq Z3 = vvv * this->Z_;             // Z3 = vvv*Z1
...
{
  /*
    return bw6_761_G2(bw6_761_twist_mul_by_q_X * (this->X_).Frobenius_map(1),
                   bw6_761_twist_mul_by_q_Y * (this->Y_).Frobenius_map(1),
...
void bw6_761_G2::batch_to_special_all_non_zeros(std::vector<bw6_761_G2> &vec)
{
    ...
    {
        vec[i] = bw6_761_G2(vec[i].X() * Z_vec[i], vec[i].Y() * Z_vec[i], one);
    }
}
