...
void sw6_G2::print() const
{
    ...
    else
    {
        ...
        gmp_printf("(%Nd*z^2 + %Nd*z + %Nd , %Nd*z^2 + %Nd*z + %Nd)\n",
                   copy.X_.c2.as_bigint().data, sw6_Fq::num_limbs,
                   copy.X_.c1.as_bigint().data, sw6_Fq::num_limbs,
                   copy.X_.c0.as_bigint().data, sw6_Fq::num_limbs,
                   copy.Y_.c2.as_bigint().data, sw6_Fq::num_limbs,
                   copy.Y_.c1.as_bigint().data, sw6_Fq::num_limbs,
                   copy.Y_.c0.as_bigint().data, sw6_Fq::num_limbs);
    }
}
...
void sw6_G2::print_coordinates() const
{
    ...
    else
    {
        gmp_printf("(%Nd*z^2 + %Nd*z + %Nd : %Nd*z^2 + %Nd*z + %Nd : %Nd*z^2 + %Nd*z + %Nd)\n",
                   this->X_.c2.as_bigint().data, sw6_Fq::num_limbs,
                   this->X_.c1.as_bigint().data, sw6_Fq::num_limbs,
                   this->X_.c0.as_bigint().data, sw6_Fq::num_limbs,
                   this->Y_.c2.as_bigint().data, sw6_Fq::num_limbs,
                   this->Y_.c1.as_bigint().data, sw6_Fq::num_limbs,
                   this->Y_.c0.as_bigint().data, sw6_Fq::num_limbs,
                   this->Z_.c2.as_bigint().data, sw6_Fq::num_limbs,
                   this->Z_.c1.as_bigint().data, sw6_Fq::num_limbs,
                   this->Z_.c0.as_bigint().data, sw6_Fq::num_limbs);
    }
}
...
void sw6_G2::to_affine_coordinates()
{
    if (this->is_zero())
    {
        this->X_ = sw6_Fq3::zero();
        this->Y_ = sw6_Fq3::one();
        this->Z_ = sw6_Fq3::zero();
    }
    else
    {
        const sw6_Fq3 Z_inv = Z_.inverse();
...
{
    return (this->is_zero() || this->Z_ == sw6_Fq3::one());
}
...
sw6_G2 sw6_G2::operator+(const sw6_G2 &other) const
{
    ...
    */
    ...
    const sw6_Fq3 X1Z2 = (this->X_) * (other.Z_);        // X1Z2 = X1*Z2
    const sw6_Fq3 X2Z1 = (this->Z_) * (other.X_);        // X2Z1 = X2*Z1

    // (used both in add and double checks)

    const sw6_Fq3 Y1Z2 = (this->Y_) * (other.Z_);        // Y1Z2 = Y1*Z2
    const sw6_Fq3 Y2Z1 = (this->Z_) * (other.Y_);        // Y2Z1 = Y2*Z1

    if (X1Z2 == X2Z1 && Y1Z2 == Y2Z1)
    {
        // perform dbl case
        const sw6_Fq3 XX   = (this->X_).squared();                   // XX  = X1^2
        const sw6_Fq3 ZZ   = (this->Z_).squared();                   // ZZ  = Z1^2
        const sw6_Fq3 w    = sw6_G2::mul_by_a(ZZ) + (XX + XX + XX); // w   = a*ZZ + 3*XX
        const sw6_Fq3 Y1Z1 = (this->Y_) * (this->Z_);
        const sw6_Fq3 s    = Y1Z1 + Y1Z1;                             // s   = 2*Y1*Z1
        const sw6_Fq3 ss   = s.squared();                             // ss  = s^2
        const sw6_Fq3 sss  = s * ss;                                  // sss = s*ss
        const sw6_Fq3 R    = (this->Y_) * s;                          // R   = Y1*s
        const sw6_Fq3 RR   = R.squared();                             // RR  = R^2
        const sw6_Fq3 B    = ((this->X_)+R).squared()-XX-RR;          // B   = (X1+R)^2 - XX - RR
    ...
    const sw6_Fq3 Z1Z2 = (this->Z_) * (other.Z_);   // Z1Z2 = Z1*Z2
...
{
    return sw6_G2(this->X_, -(this->Y_), this->Z_);
}
...
sw6_G2 sw6_G2::add(const sw6_G2 &other) const
{
    ...
    const sw6_Fq3 X1Z2 = (this->X_) * (other.Z_);        // X1Z2 = X1*Z2
    const sw6_Fq3 Z1Z2 = (this->Z_) * (other.Z_);        // Z1Z2 = Z1*Z2
    const sw6_Fq3 u    = (other.Y_) * (this->Z_) - Y1Z2; // u    = Y2*Z1-Y1Z2
    const sw6_Fq3 uu   = u.squared();                    // uu   = u^2
    const sw6_Fq3 v    = (other.X_) * (this->Z_) - X1Z2; // v    = X2*Z1-X1Z2
    const sw6_Fq3 vv   = v.squared();                    // vv   = v^2
    const sw6_Fq3 vvv  = v * vv;                         // vvv  = v*vv
    const sw6_Fq3 R    = vv * X1Z2;                      // R    = vv*X1Z2
    const sw6_Fq3 A    = uu * Z1Z2 - (vvv + R + R);      // A    = uu*Z1Z2 - vvv - 2*R
...
sw6_G2 sw6_G2::mixed_add(const sw6_G2 &other) const
{
    ...
    const sw6_Fq3 &X1Z2 = (this->X_);                   // X1Z2 = X1*Z2 (but other is special and not zero)
    const sw6_Fq3 X2Z1 = (this->Z_) * (other.X_);       // X2Z1 = X2*Z1

    // (used both in add and double checks)

    const sw6_Fq3 &Y1Z2 = (this->Y_);                   // Y1Z2 = Y1*Z2 (but other is special and not zero)
    const sw6_Fq3 Y2Z1 = (this->Z_) * (other.Y_);       // Y2Z1 = Y2*Z1
    ...
    const sw6_Fq3 u = Y2Z1 - this->Y_;             // u = Y2*Z1-Y1
    const sw6_Fq3 uu = u.squared();                // uu = u2
    const sw6_Fq3 v = X2Z1 - this->X_;             // v = X2*Z1-X1
    const sw6_Fq3 vv = v.squared();                // vv = v2
    const sw6_Fq3 vvv = v*vv;                      // vvv = v*vv
    const sw6_Fq3 R = vv * this->X_;               // R = vv*X1
    const sw6_Fq3 A = uu * this->Z_ - vvv - R - R; // A = uu*Z1-vvv-2*R
    const sw6_Fq3 X3 = v * A;                      // X3 = v*A
    const sw6_Fq3 Y3 = u*(R-A) - vvv * this->Y_;   // Y3 = u*(R-A)-vvv*Y1
    const sw6_Fq3 Z3 = vvv * this->Z_;             // Z3 = vvv*Z1
...
{
    return sw6_G2(sw6_twist_mul_by_q_X * (this->X_).Frobenius_map(1),
                   sw6_twist_mul_by_q_Y * (this->Y_).Frobenius_map(1),
...
bool sw6_G2::is_well_formed() const
{
    ...
    else
    {
        ...
        */
        const sw6_Fq3 X2 = this->X_.squared();
        const sw6_Fq3 Y2 = this->Y_.squared();
        const sw6_Fq3 Z2 = this->Z_.squared();
        const sw6_Fq3 aZ2 = sw6_twist_coeff_a * Z2;

        return (this->Z_ * (Y2 - sw6_twist_coeff_b * Z2) == this->X_ * (X2 + aZ2));
    }
}
