...
sw6_bis_Fq sw6_bis_G2::twist;
sw6_bis_Fq sw6_bis_G2::coeff_a;
sw6_bis_Fq sw6_bis_G2::coeff_b;
...
sw6_bis_Fq sw6_bis_G2::mul_by_a(const sw6_bis_Fq &elt)
{
    return sw6_bis_Fq(sw6_bis_G2::coeff_a * elt);
}

sw6_bis_Fq sw6_bis_G2::mul_by_b(const sw6_bis_Fq &elt)
{
    return sw6_bis_Fq(sw6_bis_G2::coeff_b * elt);
}
...
sw6_bis_G2 sw6_bis_G2::operator+(const sw6_bis_G2 &other) const
{
    ...
    */
    ...
    if (X1Z2 == X2Z1 && Y1Z2 == Y2Z1)
    {
        ...
        const sw6_bis_Fq w    = sw6_bis_G2::mul_by_a(ZZ) + (XX + XX + XX); // w   = a*ZZ + 3*XX
...
sw6_bis_G2 sw6_bis_G2::mixed_add(const sw6_bis_G2 &other) const
{
    ...
    const sw6_bis_Fq u = Y2Z1 - this->Y_;             // u = Y2*Z1-Y1
    const sw6_bis_Fq uu = u.squared();                // uu = u2
    const sw6_bis_Fq v = X2Z1 - this->X_;             // v = X2*Z1-X1
    const sw6_bis_Fq vv = v.squared();                // vv = v2
    const sw6_bis_Fq vvv = v*vv;                      // vvv = v*vv
    const sw6_bis_Fq R = vv * this->X_;               // R = vv*X1
    const sw6_bis_Fq A = uu * this->Z_ - vvv - R - R; // A = uu*Z1-vvv-2*R
    const sw6_bis_Fq X3 = v * A;                      // X3 = v*A
    const sw6_bis_Fq Y3 = u*(R-A) - vvv * this->Y_;   // Y3 = u*(R-A)-vvv*Y1
    const sw6_bis_Fq Z3 = vvv * this->Z_;             // Z3 = vvv*Z1
...
{
    return sw6_bis_G2::base_field_char() * (*this);
}
...
bool sw6_bis_G2::is_well_formed() const
{
    ...
    else
    {
        ...
        */
        ...
        const sw6_bis_Fq aZ2 = sw6_bis_twist_coeff_a * Z2;
...
std::istream& operator>>(std::istream &in, sw6_bis_G2 &g)
{
    ...
    if (!is_zero)
    {
        const sw6_bis_Fq tX2 = tX.squared();
        const sw6_bis_Fq tY2 = (tX2 + sw6_bis_twist_coeff_a) * tX + sw6_bis_twist_coeff_b;
        tY = tY2.sqrt();
