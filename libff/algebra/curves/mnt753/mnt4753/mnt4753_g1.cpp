...
void mnt4753_G1::print() const
{
    ...
    else
    {
        ...
        gmp_printf("(%Nd , %Nd)\n",
                   copy.X.as_bigint().data, mnt4753_Fq::num_limbs,
                   copy.Y.as_bigint().data, mnt4753_Fq::num_limbs);
    }
}
...
void mnt4753_G1::print_coordinates() const
{
    ...
    {
        gmp_printf("(%Nd : %Nd : %Nd)\n",
                   this->X.as_bigint().data, mnt4753_Fq::num_limbs,
                   this->Y.as_bigint().data, mnt4753_Fq::num_limbs,
                   this->Z.as_bigint().data, mnt4753_Fq::num_limbs);
    }
}
...
void mnt4753_G1::to_affine_coordinates()
{
    if (this->is_zero())
    {
        this->X = mnt4753_Fq::zero();
        this->Y = mnt4753_Fq::one();
        this->Z = mnt4753_Fq::zero();
    }
    else
    {
        const mnt4753_Fq Z_inv = Z.inverse();
        this->X = this->X * Z_inv;
        this->Y = this->Y * Z_inv;
        this->Z = mnt4753_Fq::one();
    }
}
...
{
    return (this->is_zero() || this->Z == mnt4753_Fq::one());
}
...
mnt4753_G1 mnt4753_G1::add(const mnt4753_G1 &other) const
{
    ...
    const mnt4753_Fq X1Z2 = (this->X) * (other.Z);        // X1Z2 = X1*Z2
    const mnt4753_Fq Z1Z2 = (this->Z) * (other.Z);        // Z1Z2 = Z1*Z2
    const mnt4753_Fq u    = (other.Y) * (this->Z) - Y1Z2; // u    = Y2*Z1-Y1Z2
    const mnt4753_Fq uu   = u.squared();                    // uu   = u^2
    const mnt4753_Fq v    = (other.X) * (this->Z) - X1Z2; // v    = X2*Z1-X1Z2
    const mnt4753_Fq vv   = v.squared();                    // vv   = v^2
    const mnt4753_Fq vvv  = v * vv;                         // vvv  = v*vv
    const mnt4753_Fq R    = vv * X1Z2;                      // R    = vv*X1Z2
    const mnt4753_Fq A    = uu * Z1Z2 - (vvv + R + R);      // A    = uu*Z1Z2 - vvv - 2*R
...
mnt4753_G1 mnt4753_G1::mixed_add(const mnt4753_G1 &other) const
{
    ...
    const mnt4753_Fq &X1Z2 = (this->X);                    // X1Z2 = X1*Z2 (but other is special and not zero)
    const mnt4753_Fq X2Z1 = (this->Z) * (other.X);        // X2Z1 = X2*Z1

    // (used both in add and double checks)

    const mnt4753_Fq &Y1Z2 = (this->Y);                    // Y1Z2 = Y1*Z2 (but other is special and not zero)
    const mnt4753_Fq Y2Z1 = (this->Z) * (other.Y);        // Y2Z1 = Y2*Z1
    ...
    const mnt4753_Fq A = uu * this->Z - vvv - R - R;  // A = uu*Z1-vvv-2*R
    const mnt4753_Fq X3 = v * A;                       // X3 = v*A
...
bool mnt4753_G1::is_well_formed() const
{
    ...
    else
    {
        ...
        */
        const mnt4753_Fq X2 = this->X.squared();
        const mnt4753_Fq Y2 = this->Y.squared();
        const mnt4753_Fq Z2 = this->Z.squared();

        return (this->Z * (Y2 - mnt4753_G1::coeff_b * Z2) == this->X * (X2 + mnt4753_G1::coeff_a * Z2));
    }
}
